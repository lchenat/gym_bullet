        ...
        for i in range(len(bodies)):
            ...
            for j in range(p.getNumJoints(bodies[i])):
                p.setJointMotorControl2(
                    ...
                    velocityGain=0.1,
                    force=0)
                _, joint_name, _, _, _, _, _, _, _, _, _, _, part_name = p.getJointInfo(
                    bodies[i], j)
...
class BodyPart:
    ...
    def state_fields_of_pose_of(
        ...
        if link_id == -1:
            ...
            (x, y, z), (a, b, c, d), _, _, _, _ = p.getLinkState(
                body_id, link_id)
    ...
    def contact_list(self):
        return p.getContactPoints(self.bodies[self.bodyIndex], -1,
...
class Joint:
    def __init__(self, joint_name, bodies, bodyIndex, jointIndex):
        ...
        _, _, _, _, _, _, _, _, self.lowerLimit, self.upperLimit, _, _, _ = p.getJointInfo(
