...
from robot_locomotors import Hopper, Walker2D, HalfCheetah, Ant, Insect, Humanoid, Swimmer


class WalkerBaseBulletEnv(MJCFBaseBulletEnv):
    def __init__(self, robot, render=False, d=None):
        self.d = d
    ...
    def _step(self, a):
        ...
        alive = float(
            self.robot.alive_bonus(state[1] + self.robot.initial_z,
        ...
        progress = np.dot((np.cos(np.pi * (self.robot.d - 0.25)), np.sin(
            np.pi * (self.robot.d - 0.25))), (self.robot.body_xyz[0],
        ...
        self.rewards = [
            alive, progress, electricity_cost, joints_at_limit_cost,
            feet_collision_cost
        ]
        if (debugmode):
            print("rewards=")
